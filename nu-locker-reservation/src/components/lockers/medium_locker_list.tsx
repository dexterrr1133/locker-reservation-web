...
const MediumLockers: FC<LockerProps> = ({ className = '' }) => {
  ...
  return (
    <Card className={`p-6 ${className}`}>
      <div className="space-y-4">
        <h3 className="text-lg font-semibold border-b pb-2">Medium Lockers</h3>
            ...
            <div key={rowIndex} className="flex gap-4 justify-center">
              {row.map((locker, colIndex) => (
                <button
                  ...
                  aria-label={`Medium locker ${colIndex + 1}${String.fromCharCode(65 + rowIndex)}`}
                >
